...
public class Main extends Application {
    ...
    private Node createEllipticShape(double radius) {
        ...
        circle.setFill(Color.rgb(random.nextInt(255), random.nextInt(255), random.nextInt(255), .55));
        return circle;
    }
    ...
    private void createStage(Stage stage) {
        ...
        root.getTransforms().add(
                Transform.translate(scene.getWidth() / 2, scene.getHeight() / 2));
        int side = scene.getWidth() < scene.getHeight() ? (int) scene.getWidth() : (int) scene.getHeight();

        for (int i = 0; i < MAX_NODES; i++) {
            Node n = createEllipticShape(random.nextInt(20));
            root.getChildren().add(n);
            int unit = random.nextInt(side);
            Path p = createOrbitalPath(unit, 0, unit - 100, unit - 100, 0);
            root.getChildren().add(p);
            createPathTransition(random.nextInt(10), p, n).play();
        }
